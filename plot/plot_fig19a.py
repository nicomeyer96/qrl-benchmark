...
if __name__ == '__main__':
    ...
    for model, model_ids, model_color, model_marker in zip(models, models_ids, models_color, models_marker):
        ...
        model_degree_avg_sampling_complexity = []
        ...
        for degree in degrees:
            ...
            sampling_complexities = evaluate_sampling_complexity_with_percentiles(
                path=os.path.join('results', 'dqn', 'a3-main', f'deg{degree}'),
                filename=model,
            ...
            model_degree_avg_sampling_complexity.append(sampling_complexities[:, :, 0])
            ...
            for _axis, _sampling_complexity, delta in zip(ax, np.transpose(sampling_complexities, (1, 0, 2)), deltas):
                for axis, sampling_complexity, epsilon in zip(_axis, _sampling_complexity, epsilons):
                    ...
                    axis.scatter(degree, sampling_complexity[0], color=model_color, marker=model_marker, alpha=alpha, s=12)
                    # axis.errorbar(degree, sampling_complexity[0], yerr=np.expand_dims(sampling_complexity[1:], axis=1),
        ...
        for _axis, _sampling_complexity, delta in zip(ax, np.transpose(model_degree_avg_sampling_complexity, (2, 1, 0)), deltas):
            for axis, sampling_complexity, epsilon in zip(_axis, _sampling_complexity, epsilons):
                axis.plot(degrees, sampling_complexity, color=model_color, linestyle='--', linewidth=.75)
    ...
    # set axis labels
    fig.supxlabel(r'$\textbf{trajectory support points}$', fontsize=8, y=-0.0)
    ...
    # save figure
    path = os.path.join(os.path.dirname(__file__), 'plots', f'fig19a.pdf')
