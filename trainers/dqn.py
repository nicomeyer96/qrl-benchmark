...
class DQNTrainer(Trainer):
    ...
    def __init__(self,
                 ...
                 qubits_qnn: Optional[int], layers_qnn: Optional[int], ansatz_qnn: Optional[str], gates_qnn: Optional[str],
                 discount_factor: float, epochs_train: int, batch_update: int, epsilon_greedy: float):
        ...
        if 'classical' == model:
            ...
            self.net = ClassicalNN(self.env.observation_space.shape, self.env.action_space.n,
            ...
            self.net = HybridNN(self.env.observation_space.shape,self.env.action_space.n,
                                qubits=qubits_qnn, layers=layers_qnn, ansatz=ansatz_qnn, gates=gates_qnn)
            # set learning rates individually for classical and quantum layers
            self.optim = torch.optim.Adam([
                     {'params': self.net.model[0].weight}, {'params': self.net.model[0].bias},
                     {'params': self.net.model[1].weights, 'lr': lr_quantum},
                     {'params': self.net.model[1].scaling, 'lr': lr_quantum},
                     {'params': self.net.model[2].weight}, {'params': self.net.model[2].bias}
            ], lr=lr_classical)
        ...
        policy = ts.policy.DQNPolicy(
            model=self.net,
            optim=self.optim,
            action_space=self.env.action_space,  # noqa  (suppress faulty WrongType warning)
            discount_factor=discount_factor,
            target_update_freq=environments_train * env_steps
        )
        ...
        self.trainer = ts.trainer.OffpolicyTrainer(
            ...
            train_fn=lambda epoch, env_step: policy.set_eps(epsilon_greedy),
